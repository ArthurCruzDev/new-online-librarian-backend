...
use super::collection_repository::CollectionRepository;

#[derive(Clone)]
pub struct CollectionRepositoryMySQL {
    connection: Arc<MySqlPool>,
}

impl CollectionRepositoryMySQL {
...
impl CollectionRepository for CollectionRepositoryMySQL {
    async fn save(&self, location: &Collection) -> Result<Option<Collection>, sqlx::Error> {
        match location.id {
            Some(_) => todo!(),
            None => {
    ...
    async fn find_by_id(&self, id: u64) -> Result<Option<Collection>, sqlx::Error> {
    ...
    async fn find_all_by_user_id(&self, user_id: u64) -> Result<Vec<Collection>, sqlx::Error> {
        let query_result = sqlx::query!(
            r#"
            SELECT *
            FROM collections u
